...
// Selecciona el contenedor donde se mostrarán las tarjetas de los carros
let tarjetasCarros = document.querySelector('.contenedorTarjeta');
...
// es para crear la tarjeta de los carro
function crearTarjeta(carro) {
  return `
    <div class="contenedorTarjetas">
      <img class="imgtargetas" src="${carro.imagen}" alt="${carro.nombre}">
      <h2>${carro.nombre}</h2>
      <p>Categoria: ${carro.categoriaCarro}</p>
      <p>Precio: ${carro.precioAlquiler}</p>
      <p>Codigo: ${carro.codigo}</p>
      <p>Lujo: ${carro.caracteristicasDestacadas.lujo}</p>
      <p>Comodidad: ${carro.caracteristicasDestacadas.comodidad}</p>
    </div>
  `;
}
...
function mostrarPagina(page) {
  // Calcula el índice de inicio y fin para las tarjetas a mostrar en la página actual
  const inicioIndex = (page - 1) * elementosPorPagina;
  const finIndex = page * elementosPorPagina;

  // Limpia el contenedor para poner las nuevas tarjetas
  tarjetasCarros.innerHTML = '';

  // Itera sobre los elementos de la página actual y agrega las tarjetas al contenedor
  for (let i = inicioIndex; i < finIndex && i < carros.length; i++) {
    const carro = carros[i];
    const tarjetaHTML = crearTarjeta(carro);
    tarjetasCarros.innerHTML += tarjetaHTML;
  }

  // muestra la página en la que se esta
...
function actualizarBotonesPaginacion() {
  ...
  const totalPaginas = Math.ceil(carros.length / elementosPorPagina);

  // esconde el botón anterior si estamos en la primera página
  if (paginaActual === 1) {
    document.getElementById('paginaAnterior').style.display = 'none';
  } else {
    document.getElementById('paginaAnterior').style.display = 'inline-block';
  }

  // esconde el botón siguiente si estamos en la última página
  if (paginaActual >= totalPaginas) {
    document.getElementById('paginaSiguiente').style.display = 'none';
  } else {
    document.getElementById('paginaSiguiente').style.display = 'inline-block';
  }
}

// para ir a la página anterior
function irAnteriorPagina() {
  // se va  bajando el numero de la pagina para llegar a la pagina 1
  if (paginaActual > 1) {
    paginaActual--;
...
// con esto se recibe datos del registro
if (sessionStorage.getItem('nuevoCarro')) {
  // Si hay un nuevo carro en el sessionStorage, lo añade a la lista de carros
  const nuevoCarro = JSON.parse(sessionStorage.getItem('nuevoCarro'));
  carros.push(nuevoCarro);
  // Limpia el nuevo carro del sessionStorage
  sessionStorage.removeItem('nuevoCarro');
