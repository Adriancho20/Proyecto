...
document.addEventListener('DOMContentLoaded', function() {
  ...
  videoFondo.playbackRate = 0.5; //  velocidad del video 
});
...
function registroProducto() {
  ...
  const precioAlquiler = document.getElementById('Precio').value;
  const categoria = document.querySelector('.Categoria').value;
  const selectorImg = document.querySelector('.selectorImg');
  const imagen = selectorImg.options[selectorImg.selectedIndex].getAttribute('data-img-src');
  const lujo = document.querySelector('.SelectorLujo').value;
  const rangoVelocidad = document.querySelector('.SelectorRangoVelocidad').value;
  const comodidad = document.querySelector('.SelectorComodidad').value;

  // validar el código y el precio
  const caracteresCodigo = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{8,}$/;
  const caracteresPrecio = /^[0-9]+(\.[0-9]+)?$/;

  
  if (nombre.length > 20) {
    alert('El nombre no debe exceder los 20 caracteres.');
    window.location.replace("./vista2/vista2.html");
    return false;
  }


  if (!caracteresCodigo.test(codigo)) {
    alert('El código debe tener al menos 1 minúscula, 1 mayúscula, 1 número y ser de 8 caracteres o más.');
    window.location.replace("./vista2/vista2.html");
    return false;
  }

  
  if (!caracteresPrecio.test(precioAlquiler)) {
    alert('El precio solo debe tener números y puntos para separar los miles.');
    window.location.replace("./vista2/vista2.html");
    return false;
  }

  // Crear un objeto con la información del nuevo carro
  const nuevoCarro = {
    nombre,
    codigo,
    precioAlquiler,
  ...
  // Guardar la información del nuevo carro en sessionStorage
  sessionStorage.setItem('nuevoCarro', JSON.stringify(nuevoCarro));
  alert('La información se ha guardado correctamente.');

  window.location.replace("./vista3/vista3.html");
...
function limpiarInputs() {
  ...
  const selectores = document.querySelectorAll('.Categoria, .selectorImg, .SelectorLujo, .SelectorRangoVelocidad, .SelectorComodidad');
  // Restablecer el índice seleccionado de los selectores
  selectores.forEach(selector => selector.selectedIndex = -1);
}
