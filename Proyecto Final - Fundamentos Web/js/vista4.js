let carros = [
    {
      ...
      imagen: "https://trackuza.com/cdn/shop/products/LBWORKS_NISSAN_GT-R_TYPE2_60.jpeg?v=1555935888&width=1080",
      codigo: "N1GtR2023",
    ...
    {
      nombre: "Limusina Aurus Senat L700",
      categoriaCarro: "Limusina",
    ...
    {
      nombre: "Lexus LM 300h Luxury Minivan",
      categoriaCarro: "Minivan",
...
document.addEventListener('DOMContentLoaded', function() {
  // Selecciona elementos del DOM por su clase
  const Buscar = document.querySelector('.botonesvista4');
  const Limpiar = document.querySelector('.botonesvistalim4');
  // Obtiene el cuerpo de la tabla por su ID
  const Tabla = document.getElementById('table').getElementsByTagName('tbody')[0];
  // carga
  const msgCargando = document.getElementById('loading');
  let datosTabla = [];
  let paginaActual = 1;
  const filasPorPagina = 10;

  // crear una fila de tabla con los datos del carro
  function createTableRow(carro) {
    const nuevafila = document.createElement('tr');
    nuevafila.innerHTML = `
      <td class="fila1"><img class="imagenCarro" src="${carro.imagen}" alt="${carro.nombre}"></td>
      <td class="fila2">${carro.nombre}</td>
      <td class="fila3">${carro.categoriaCarro}</td>
      <td class="fila4">${carro.codigo}</td>
      <td class="fila5">${carro.precioAlquiler}</td>
      <td class="fila6">${carro.caracteristicasDestacadas.lujo}</td>
      <td class="fila7">${carro.caracteristicasDestacadas.rangoVelocidad}</td>
      <td class="fila8">${carro.caracteristicasDestacadas.comodidad}</td>
    `;
    return nuevafila;
  }

  // para filtrar carros basada en las selecciones del usuario
  function filterCars() {
    // se obtiene valores seleccionados por el usuario
    const selectorCategoria = document.querySelector('.Categoria4').value;
    const selectorComodidad = document.querySelector('.SelectorComodidad4').value;
    const selectorLujo = document.querySelector('.SelectorLujo4').value; // Obtiene la selección de lujo
  
    // filtra los carros con lo seleccionado
    return carros.filter(carro => {
      // Verifica si coincide la categoría, comodidad, lujo
      const categoriaCoincide = selectorCategoria === 'En blanco' || carro.categoriaCarro === selectorCategoria; 
      const comodidadCoincide = selectorComodidad === 'En blanco' || carro.caracteristicasDestacadas.comodidad === selectorComodidad;
      const lujoCoincide = selectorLujo === 'En blanco' || carro.caracteristicasDestacadas.lujo === selectorLujo; // Incluye la verificación de lujo
  
      return categoriaCoincide && comodidadCoincide && lujoCoincide;
    });
  }


  function tarjetasCarros(filtroCarros) {
    // actualiza la info con los coches filtrados
    datosTabla = filtroCarros;
    paginaActual = 1;
    actualizarTabla();
  }

  // para actualizar la tabla con los datos de la paginacion
  function actualizarTabla() {
    ...
    datosPaginados.forEach(carro => Tabla.appendChild(createTableRow(carro)));
  ...
  function cambiarPagina(nuevaPagina) {
    ...
    actualizarTabla();
    // actualiza el numero de la paginacion
  ...
  // eventos a los botones de paginación
  ...
  // para limpiar los datos de la tabla
  function clearTable() {
    while (Tabla.firstChild && Tabla.firstChild.nextSibling) {
      Tabla.removeChild(Tabla.firstChild.nextSibling);
    }
    
    // restablece los campos de selección apenas se filtra
    document.querySelector('.Categoria4').selectedIndex = 0;
    document.querySelector('.SelectorComodidad4').selectedIndex = 0;
    document.querySelector('.SelectorLujo4').selectedIndex = 0;
  ...
  // escuchador de eventos de clic al botón de búsqueda
  Buscar.addEventListener('click', function() {
    // msj de carga
    msgCargando.textContent = 'Cargando...';
    msgCargando.style.display = 'block';

    // simula la recoleccion de datos con una promesa
    const fetchPromise = new Promise((resolve, reject) => {
      setTimeout(() => {
        // recolecta los carros filtrados 
        const filtroCarros = filterCars();

        // se rsuelve la promesa
        resolve(filtroCarros);
      }, 2000); // tiempo de espera
    });

    // se actualiza la tabla después de que se resuelva la promesa
    fetchPromise
      .then(filtroCarros => {
        // quitamos el msj de carga
        msgCargando.style.display = 'none';

        // mostramos los carros filtrados en la tabla
        tarjetasCarros(filtroCarros);
      })
      .catch(error => {
    ...
    // escuchador de eventos de clic al botón de limpiar
    Limpiar.addEventListener('click', clearTable);
